// Generated by the protocol buffer compiler.  DO NOT EDIT!
// NO CHECKED-IN PROTOBUF GENCODE
// source: salto/nebula/type/remote_operation.proto
// Protobuf Java Version: 4.33.6

package com.saltoapis.nebula.type;

/**
 * <pre>
 * Specifies the remote operation supported by the app key.
 * </pre>
 *
 * Protobuf enum {@code salto.nebula.type.AppKeyRemoteOperation}
 */
@com.google.protobuf.Generated
public enum AppKeyRemoteOperation
    implements com.google.protobuf.ProtocolMessageEnum {
  /**
   * <pre>
   * Unspecified or unknown remote operation.
   * </pre>
   *
   * <code>APP_KEY_REMOTE_OPERATION_UNSPECIFIED = 0;</code>
   */
  APP_KEY_REMOTE_OPERATION_UNSPECIFIED(0),
  /**
   * <pre>
   * Locks the access point remotely.
   * </pre>
   *
   * <code>LOCK = 1;</code>
   */
  LOCK(1),
  /**
   * <pre>
   * Unlocks the access point remotely.
   * </pre>
   *
   * <code>UNLOCK = 2;</code>
   */
  UNLOCK(2),
  UNRECOGNIZED(-1),
  ;

  static {
    com.google.protobuf.RuntimeVersion.validateProtobufGencodeVersion(
      com.google.protobuf.RuntimeVersion.RuntimeDomain.PUBLIC,
      /* major= */ 4,
      /* minor= */ 33,
      /* patch= */ 6,
      /* suffix= */ "",
      "AppKeyRemoteOperation");
  }
  /**
   * <pre>
   * Unspecified or unknown remote operation.
   * </pre>
   *
   * <code>APP_KEY_REMOTE_OPERATION_UNSPECIFIED = 0;</code>
   */
  public static final int APP_KEY_REMOTE_OPERATION_UNSPECIFIED_VALUE = 0;
  /**
   * <pre>
   * Locks the access point remotely.
   * </pre>
   *
   * <code>LOCK = 1;</code>
   */
  public static final int LOCK_VALUE = 1;
  /**
   * <pre>
   * Unlocks the access point remotely.
   * </pre>
   *
   * <code>UNLOCK = 2;</code>
   */
  public static final int UNLOCK_VALUE = 2;


  public final int getNumber() {
    if (this == UNRECOGNIZED) {
      throw new java.lang.IllegalArgumentException(
          "Can't get the number of an unknown enum value.");
    }
    return value;
  }

  /**
   * @param value The numeric wire value of the corresponding enum entry.
   * @return The enum associated with the given numeric wire value.
   * @deprecated Use {@link #forNumber(int)} instead.
   */
  @java.lang.Deprecated
  public static AppKeyRemoteOperation valueOf(int value) {
    return forNumber(value);
  }

  /**
   * @param value The numeric wire value of the corresponding enum entry.
   * @return The enum associated with the given numeric wire value.
   */
  public static AppKeyRemoteOperation forNumber(int value) {
    switch (value) {
      case 0: return APP_KEY_REMOTE_OPERATION_UNSPECIFIED;
      case 1: return LOCK;
      case 2: return UNLOCK;
      default: return null;
    }
  }

  public static com.google.protobuf.Internal.EnumLiteMap<AppKeyRemoteOperation>
      internalGetValueMap() {
    return internalValueMap;
  }
  private static final com.google.protobuf.Internal.EnumLiteMap<
      AppKeyRemoteOperation> internalValueMap =
        new com.google.protobuf.Internal.EnumLiteMap<AppKeyRemoteOperation>() {
          public AppKeyRemoteOperation findValueByNumber(int number) {
            return AppKeyRemoteOperation.forNumber(number);
          }
        };

  public final com.google.protobuf.Descriptors.EnumValueDescriptor
      getValueDescriptor() {
    if (this == UNRECOGNIZED) {
      throw new java.lang.IllegalStateException(
          "Can't get the descriptor of an unrecognized enum value.");
    }
    return getDescriptor().getValues().get(ordinal());
  }
  public final com.google.protobuf.Descriptors.EnumDescriptor
      getDescriptorForType() {
    return getDescriptor();
  }
  public static com.google.protobuf.Descriptors.EnumDescriptor
      getDescriptor() {
    return com.saltoapis.nebula.type.RemoteOperationProto.getDescriptor().getEnumTypes().get(0);
  }

  private static final AppKeyRemoteOperation[] VALUES = values();

  public static AppKeyRemoteOperation valueOf(
      com.google.protobuf.Descriptors.EnumValueDescriptor desc) {
    if (desc.getType() != getDescriptor()) {
      throw new java.lang.IllegalArgumentException(
        "EnumValueDescriptor is not for this type.");
    }
    if (desc.getIndex() == -1) {
      return UNRECOGNIZED;
    }
    return VALUES[desc.getIndex()];
  }

  private final int value;

  private AppKeyRemoteOperation(int value) {
    this.value = value;
  }

  // @@protoc_insertion_point(enum_scope:salto.nebula.type.AppKeyRemoteOperation)
}

